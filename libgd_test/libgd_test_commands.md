...
Для автоматизации сборки статической библиотеки можно воспользоваться скриптом [build_libgd.sh](build_libgd.sh).

Скрипт должен находиться в корневом каталоге проекта libgd - рядом с файлом `CMakeLists.txt`
